...
execute as @a[predicate=!sitanywhere:is_sneaking] if score @s sitanywhere_sneak_parameter matches 1.. run scoreboard players remove @s sitanywhere_sneak_parameter 1
execute as @a[predicate=sitanywhere:is_sneaking,x_rotation=85..90] at @s unless entity @e[type=interaction,tag=sit,tag=global,distance=...1] run function sitanywhere:global_sit_init
...
# create a command that detect if a player and an entity are on the same block
execute as @a at @s if entity @e[type=interaction,tag=sit,tag=stair_or_slab,distance=..1] positioned ~ ~-0.1 ~ align xyz run execute as @e[type=interaction,tag=sit,tag=stair_or_slab,distance=..1] at @s if block ~ ~-.1 ~ #sitanywhere:stair_or_slab
